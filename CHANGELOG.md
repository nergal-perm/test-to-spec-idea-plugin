...
### Fixed
- Spec file gutter icons now appear in multi-module Maven/Gradle projects where the root aggregator directory is not registered as an IntelliJ content root; `deriveCapability` falls back to `project.basePath` when no content root matches
